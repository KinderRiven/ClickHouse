...
namespace
{
    ...
    std::shared_ptr<S3::ProxyResolverConfiguration>
    getProxyResolverConfiguration(const String & prefix, const Poco::Util::AbstractConfiguration & proxy_resolver_config)
    {
        ...
        LOG_DEBUG(
            ...
            "Configured proxy resolver: {}, Scheme: {}, Port: {}",
            endpoint.toString(),
            proxy_scheme,
            proxy_port);
    ...
    std::shared_ptr<S3::ProxyListConfiguration>
    getProxyListConfiguration(const String & prefix, const Poco::Util::AbstractConfiguration & proxy_config)
    {
    ...
    getClient(const Poco::Util::AbstractConfiguration & config, const String & config_prefix, ContextPtr context)
    {
        ...
        if (proxy_config)
        {
            ...
            client_configuration.error_report = [proxy_config](const auto & request_config) { proxy_config->errorReport(request_config); };
        }
...
void registerDiskS3(DiskFactory & factory)
{
    auto creator = [](const String & name,
        ...
        std::shared_ptr<IDisk> s3disk = std::make_shared<DiskS3>(
            name, uri.bucket, uri.key, metadata_path, context, getSettings(config, config_prefix, context), getSettings);
        ...
        if (cache_enabled)
        {
            ...
            String slice_cache_path = config.getString(config_prefix + ".cache_path", context->getPath() + "disks/" + name + "/slice/");
            ...
            auto slice_cache_disk = std::make_shared<DiskLocal>("s3-slice-cache", slice_cache_path, 0);
