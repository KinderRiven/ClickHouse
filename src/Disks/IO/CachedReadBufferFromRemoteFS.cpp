    ...
    , enable_logging(!query_id.empty() && CurrentThread::get().getQueryContext()->getSettingsRef().enable_filesystem_cache_log)
{
    ...
    LOG_INFO(
        log,
        "create cacahed_read_buffer [key:{}][file_offset_of_buffer_end:{}][read_until_position:{}][size:{}]",
        keyToStr(cache_key),
...
void CachedReadBufferFromRemoteFS::initialize(size_t offset, size_t size)
{
    ...
    {
        file_segments_holder.emplace(cache->get(cache_key, offset, size, file_size));
    }
    else
    {
        file_segments_holder.emplace(cache->getOrSet(cache_key, offset, size, file_size));
    }
    ...
    /// remove unnecessary file_segment
    for (auto iter = file_segments_holder->file_segments.begin(); iter != file_segments_holder->file_segments.end(); iter++)
    {
#ifdef NEED_TO_PRINT
        LOG_INFO(
            log,
            "need to read file_segment [key:{}][offset:{}][size:{}][state:{}]",
            keyToStr((*iter)->key()),
            (*iter)->offset(),
            (*iter)->range().size(),
            (*iter)->stateToString((*iter)->state()));
#endif

        if (file_offset_of_buffer_end >= (*iter)->range().left)
        {
#ifdef NEED_TO_PRINT
            LOG_INFO(
                log,
                "skip unnecessary file_segment, now in [key:{}][offset:{}][size:{}][state:{}]",
                keyToStr((*iter)->key()),
                (*iter)->offset(),
                (*iter)->range().size(),
                (*iter)->stateToString((*iter)->state()));
#endif
            current_file_segment_it = iter;
        }

        if ((file_offset_of_buffer_end + size) <= (*iter)->range().left)
        {
            file_segments_holder->file_segments.erase(iter, file_segments_holder->file_segments.end());
            break;
        }
    }
    if (current_file_segment_it != file_segments_holder->file_segments.begin())
    {
        file_segments_holder->file_segments.erase(file_segments_holder->file_segments.begin(), current_file_segment_it);
    }
    initialized = true;
}

bool CachedReadBufferFromRemoteFS::canDownloadFromRmoteCache(size_t offset, size_t size)
{
...
SeekableReadBufferPtr CachedReadBufferFromRemoteFS::getRemoteCacheReadBuffer(FileSegmentPtr & file_segment, ReadType read_type_)
{
    switch (read_type_)
    {
        ...
        default:
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot use remote cache reader with read type: {}", toString(read_type));
    }
}
...
SeekableReadBufferPtr CachedReadBufferFromRemoteFS::getImplementationBuffer(FileSegmentPtr & file_segment)
{
    ...
    /// [1] local cached buffer      - CACHED
    /// [2] read from remote cache   - REMOTE_CACHE_READ_AND_PUT_IN_CACHE
    /// [3] read from remote store   - REMOTE_FS_READ_AND_PUT_IN_CACHE
    /// [4] bypass local cache       - REMOTE_FS_READ_BYPASS_CACHE
    auto read_buffer_for_file_segment = getReadBufferForFileSegment(file_segment);

    [[maybe_unused]] auto download_current_segment
        = ((read_type == ReadType::REMOTE_FS_READ_AND_PUT_IN_CACHE) || (read_type == ReadType::REMOTE_CACHE_READ_AND_PUT_IN_CACHE));
    ...
    switch (read_type)
    {
        ...
        case ReadType::REMOTE_CACHE_READ_AND_PUT_IN_CACHE: {
            ...
            {
                size_t download_offset = file_segment->getDownloadedSize();
        ...
        case ReadType::REMOTE_FS_READ_AND_PUT_IN_CACHE: {
            ...
            if (download_offset != static_cast<size_t>(read_buffer_for_file_segment->getPosition()))
            {
                ...
                throw Exception(
                    ErrorCodes::LOGICAL_ERROR,
                    "Buffer's offsets mismatch; cached buffer offset: {}, download_offset: {}, position: {}, file segment info: {}",
                    file_offset_of_buffer_end,
                    download_offset,
                ...
                if (result)
                {
                    ...
                    if (read_type == ReadType::REMOTE_FS_READ_AND_PUT_IN_CACHE)
                    {
                        ...
                        if (download_offset != static_cast<size_t>(implementation_buffer->getPosition())
                            ...
                            throw Exception(
                                ErrorCodes::LOGICAL_ERROR,
                                "Buffer's offsets mismatch after predownloading; download offset: {}, cached buffer offset: {}, "
                                "implementation "
                                "buffer offset: {}, "
                                "file segment info: {}",
                                download_offset,
                    ...
                    else if (read_type == ReadType::REMOTE_CACHE_READ_AND_PUT_IN_CACHE)
                    {
                        auto download_offset = file_segment->getDownloadedSize();
                        if (download_offset != static_cast<size_t>(implementation_buffer->getPosition())
                            || file_segment->getDownloadOffset() != file_offset_of_buffer_end)
                            throw Exception(
                                ErrorCodes::LOGICAL_ERROR,
                                "Buffer's offsets mismatch after predownloading; download offset: {}, cached buffer offset: {}, "
                                "implementation "
...
bool CachedReadBufferFromRemoteFS::updateImplementationBufferIfNeeded()
{
    ...
    if ((read_type == ReadType::REMOTE_FS_READ_AND_PUT_IN_CACHE) || (read_type == ReadType::REMOTE_CACHE_READ_AND_PUT_IN_CACHE))
    {
        /**
...
bool CachedReadBufferFromRemoteFS::nextImplStep()
{
    ...
    if (needed_to_predownload)
    {
        ...
        /// Whether has data to continue read.
        result = implementation_buffer->hasPendingData();
        /// How many bytes can continue to read.
        size = implementation_buffer->available();
    }

    auto download_current_segment
        = ((read_type == ReadType::REMOTE_FS_READ_AND_PUT_IN_CACHE) || (read_type == ReadType::REMOTE_CACHE_READ_AND_PUT_IN_CACHE));
    ...
    if (result)
    {
        ...
        LOG_INFO(
            log,
            "nextImplStep [key:{}][file_offset_of_buffer_end:{}][read_until_position:{}][size:{}]",
            keyToStr(cache_key),
            file_offset_of_buffer_end,
            read_until_position,
            file_size);
...
off_t CachedReadBufferFromRemoteFS::seek(off_t offset, int whence)
{
    ...
    LOG_INFO(
        log,
        "seek_to [key:{}][file_offset_of_buffer_end:{}][read_until_position:{}][size:{}]",
        keyToStr(cache_key),
        file_offset_of_buffer_end,
        read_until_position,
        file_size);
