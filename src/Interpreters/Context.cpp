...
struct ContextSharedPart
{
    ...
    ContextSharedPart() : access_control_manager(std::make_unique<AccessControlManager>()), macros(std::make_unique<Macros>())
    {
...
void Context::setBackupsVolume(const String & path, const String & policy_name)
{
    ...
    else
    {
        ...
        if (policy->getVolumes().size() != 1)
            throw Exception(
                "Policy " + policy_name + " is used for backups, such policy should have exactly one volume",
                ErrorCodes::NO_ELEMENTS_IN_CONFIG);
...
static void reloadZooKeeperIfChangedImpl(
    const ConfigurationPtr & config, const std::string & config_name, zkutil::ZooKeeperPtr & zk, std::shared_ptr<ZooKeeperLog> zk_log)
{
...
void Context::initializeBackgroundExecutorsIfNeeded()
{
    ...
    const size_t max_merges_and_mutations
        = getSettingsRef().background_pool_size * getSettingsRef().background_merges_mutations_concurrency_ratio;
    ...
    LOG_INFO(
        shared->log,
        "Initialized background executor for merges and mutations with num_threads={}, num_tasks={}",
        getSettingsRef().background_pool_size,
        max_merges_and_mutations);
