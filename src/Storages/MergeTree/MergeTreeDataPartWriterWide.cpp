...
void MergeTreeDataPartWriterWide::addStreams(const NameAndTypePair & column, const ASTPtr & effective_codec_desc)
{
...
ISerialization::OutputStreamGetter
MergeTreeDataPartWriterWide::createStreamGetter(const NameAndTypePair & column, WrittenOffsetColumns & offset_columns) const
{
...
{
    /// ---------------------------------------------------------------------------
    /// | offset_in_compress_file | offset_in_decompress_block | num_rows_in_mark |
    /// ---------------------------------------------------------------------------
...
StreamsWithMarks MergeTreeDataPartWriterWide::getCurrentMarksForColumn(
    const NameAndTypePair & column, WrittenOffsetColumns & offset_columns, ISerialization::SubstreamPath & path)
{
    StreamsWithMarks result;
    serializations[column.name]->enumerateStreams(
    ...
    const Granule & granule)
{
    ...
    serialization->serializeBinaryBulkWithMultipleStreams(
        column, granule.start_row, granule.rows_to_write, serialize_settings, serialization_state);
    ...
    const NameAndTypePair & name_and_type, const IColumn & column, WrittenOffsetColumns & offset_columns, const Granules & granules)
{
    ...
    size_t finished_granule = 0;
    size_t total_granule = granules.size();
    for (const auto & granule : granules)
    {
        ...
        if (granule.is_complete)
        {
            ...
            for (const auto & mark : marks_it->second)
            {
                ...
                LOG_TRACE(
                    trace_log,
                    "[2][mark_info][offset_in_compress_file:{}][offset_in_decompressed_block:{}]",
                    mark.mark.offset_in_compressed_file,
                    mark.mark.offset_in_decompressed_block);
                /// new mark info
                LOG_TRACE(
                    trace_log,
